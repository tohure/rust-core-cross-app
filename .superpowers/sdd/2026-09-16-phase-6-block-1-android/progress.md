...
Task 8: complete — rojo verificado antes (con `"ABCD"` el campo guardaba `"abcd"`), verde después. Las tres premisas del plan verificadas una por una y las tres ciertas: `docs/ui-spec.md:233` acepta sólo `[0-9a-f]`, iOS lo cumple con `/^[0-9a-f]*$/`, y Android aceptaba `A-F` y normalizaba. El que diverge es Android, así que cede Android.
Task 9: Hallazgo (octavo error del plan, y es un valor esperado): el Step 2 manda asertar `assertEquals("3200.50", accounts[1].balance)`. El contrato dice **1200.50** (`cuentas_iniciales[1].saldo`). Lo caza el propio Step 2, que ordena verificar el valor contra `cases.json` antes de escribirlo — se verificó y se usó el del contrato. Si se hubiera copiado del plan, el test habría quedado rojo contra un valor inventado, que es exactamente lo que CLAUDE.md prohíbe: doblar el contrato para que pase el código.
Task 9: complete — el default de `nextEncrypt` pasa al hex de 64 de `tj-001` (verificado leyendo `cases.json`, no copiado del plan) y `AssetSourcesTest` aserta el segundo saldo, que era el único campo de DINERO de esa cuenta sin comprobar.

Task 10: complete — los dos APK pesados (31,2 MiB debug / 24,4 MiB release) con el desglose. El Step 3 (percentiles en release) NO se hizo, y se dice por qué en TESTING.md en vez de inventar un número: AGP emite el release sin firmar, y el único aparato disponible era un emulador — compararlo contra los 444 µs, que salieron de un Pixel 6 físico, daría una comparación falsa. Hallazgo de paso: el `buildTypes.release` lleva `optimization { enable = false }`, o sea que R8 no corre y el APK de release NO está minificado; por eso baja sólo 22 %. Citar ese tamaño como «lo que pesa la app» se equivoca por un factor grande, y eso no estaba dicho en ningún lado.
Task 11: complete — `docs/cross-app-pending.md` creado como dueño único de lo transversal, los cuatro PENDING recortados y apuntando ahí, el diagrama de `apps/android/README.md` reescrito con los dos módulos y las cuatro suites, los comandos del README con los totales REALES (30/1/4/19 = 54), la estructura de `CONTEXT.md` reescrita, los dos textos de `docs/ui-spec.md` corregidos y la Fase 6 agregada a `CLAUDE.md`.
Task 11: Ruling: se amplió el alcance del Step 3 del plan. Además de los dos textos que pedía, el ui-spec pasó a normar el **formato decimal de los tiempos del benchmark** (punto, independiente del locale). Motivo: la Task 7 corrigió esa divergencia en Android y el ui-spec no decía nada al respecto, así que nada impedía que la próxima app la reintrodujera. Una divergencia que se corrige en el código y no en la spec normativa vuelve.
Task 11: Ruling: el subtítulo del bloque de pegado dejó de listar plataformas — pasó de «el hex que produjo la app de iOS, React Native o Angular» a «el hex que produjo cualquiera de las otras apps». El plan sólo pedía arreglar que se autolistara en iOS; se eligió quitar la lista entera porque enumerarlas obliga a mantener el texto cada vez que se agrega una plataforma, y el defecto reaparece. Costo si me equivoco: el texto es menos concreto.
Task 11: Corrección de dos mentiras encontradas en los PENDING: el de Android decía que `ndk.abiFilters` «no está aplicado todavía» y está aplicado hace tiempo, en los dos módulos; el de React Native declaraba abierta la reverificación de Android e iOS, que se cerró en la Task 14 de la Fase 5 y que esta fase volvió a confirmar por tercera vez.
